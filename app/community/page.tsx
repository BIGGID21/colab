...
export default function CommunityFeedPage() {
  ...
  // RESTORED: removeMedia
  const removeMedia = (indexToRemove: number) => {
    setPostMedia(prev => prev.filter((_, index) => index !== indexToRemove));
  };

  // Scroll Listener for Bottom Navbar
  useEffect(() => {
    const controlNavbar = () => {
      if (typeof window !== 'undefined') {
        const currentScrollY = window.scrollY;
        // Haptic on pull-to-refresh area
        if (currentScrollY < -50) triggerHaptic(5);
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of files) {
        ...
        await supabase.storage.from('workspace_files').upload(fileName, file);
      ...
      <div key={c.id} className={`flex gap-3 text-sm ${depth > 0 ? 'ml-8 mt-3' : 'mt-4 text-left'}`}>
        ...
        <div className="flex-grow min-w-0">
          ...
          <div className="flex items-center gap-4 mt-1.5 ml-1 text-xs font-bold text-zinc-500">
            <button onClick={() => { setReplyTo({ commentId: c.id, userName: c.profiles?.full_name }); document.getElementById(`comment-input-${postId}`)?.focus(); }} className="hover:text-black dark:hover:text-white transition-colors">Reply</button>
    ...
    <div className="min-h-screen bg-white dark:bg-[#050505] transition-colors duration-300 pb-28 sm:pb-24">
      ...
      {expandedMedia && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4" onClick={() => setExpandedMedia(null)}>
          <button className="absolute top-6 right-6 text-white p-2 hover:bg-white/10 rounded-full transition-colors z-50"><X size={32} /></button>
      ...
      <header className="hidden sm:block bg-white dark:bg-[#0a0a0a] border-b border-zinc-200 dark:border-zinc-900 px-6 py-4 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="text-left">
            <h1 className="text-xl font-bold text-black dark:text-white tracking-tight flex items-center gap-2"><Globe className="text-[#9cf822]" size={20} /> Community Feed</h1>
            <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1">Global Activity</p>
          </div>
          <Link href="/dashboard" className="px-4 py-2 bg-zinc-100 dark:bg-zinc-900 text-sm font-bold rounded-xl text-black dark:text-white hover:bg-zinc-200 transition-colors">Dashboard</Link>
        </div>
      </header>
        ...
        <div className="lg:col-span-8 space-y-0 sm:space-y-6 order-2 lg:order-1">
          ...
          <div className="bg-white dark:bg-[#0a0a0a] sm:rounded-[2.5rem] p-4 sm:p-8 border-b sm:border border-zinc-200 dark:border-zinc-800 shadow-sm text-left">
            <form onSubmit={handlePost}>
              <div className="flex gap-4">
                <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 border border-zinc-200 dark:border-zinc-800"><img src={profile?.avatar_url} className="w-full h-full object-cover" /></div>
                <div className="flex-grow">
                  <textarea ref={textAreaRef} value={newPost} onChange={(e) => setNewPost(e.target.value)} placeholder="What are you building today?" className="w-full bg-transparent resize-none text-black dark:text-white text-lg focus:outline-none min-h-[80px]" />
                  {postMedia.length > 0 && (
                    <div className={`mt-3 overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 ${postMedia.length > 1 ? 'grid gap-0.5 grid-cols-2 bg-zinc-200 dark:bg-zinc-800' : ''}`}>
                      {postMedia.map((m, idx) => (
                        <div key={idx} className={`relative bg-black w-full flex justify-center items-center rounded-2xl overflow-hidden ${m.type === 'video' ? 'h-auto max-h-[800px]' : 'aspect-[4/5]'}`}>
                          {m.type === 'video' ? <video src={m.url} className="w-full h-auto max-h-[800px] object-contain rounded-2xl" autoPlay muted loop playsInline /> : <img src={m.url} className="w-full h-full object-cover" />}
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-900">
                ...
                <button type="submit" disabled={isPosting || (!newPost.trim() && postMedia.length === 0)} className="px-8 py-3 bg-[#9cf822] text-black font-black text-xs rounded-2xl active:scale-95 shadow-lg shadow-[#9cf822]/20 uppercase tracking-widest">Post Update</button>
              </div>
            </form>
          </div>
          ...
          <div className="space-y-0 sm:space-y-6">
            {posts.map((post) => {
              ...
              return (
                <div key={post.id} className={`sm:rounded-[2.5rem] p-4 sm:p-8 border-b sm:border relative overflow-hidden text-left transition-all ${isOfficial ? 'bg-zinc-50 dark:bg-[#9cf822]/[0.02] border-[#9cf822] dark:border-[#9cf822]/40 shadow-[0_0_20px_rgba(156,248,34,0.05)]' : 'bg-white dark:bg-[#0a0a0a] border-zinc-200 dark:border-zinc-800 shadow-sm'}`}>
                  ...
                  <div className="flex items-start gap-4">
                    <Link href={`/profile/${post.user_id}`} className="shrink-0 mt-1">
                      <div className={`w-12 h-12 rounded-full overflow-hidden bg-zinc-800 border-2 transition-colors ${isOfficial ? 'border-[#9cf822]' : 'border-transparent hover:border-[#9cf822]'}`}><img src={post.profiles?.avatar_url} className="w-full h-full object-cover" /></div>
                    </Link>
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {user?.id === post.user_id && <button onClick={() => handleDeletePost(post.id)} className="text-zinc-400 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>}
                      </div>
                        ...
                        <div className="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-900 animate-in slide-in-from-top-2">
                          ...
                          <div className="flex gap-2 items-end">
                            <div className="flex-grow flex flex-col gap-2">
                              ...
                              <input id={`comment-input-${post.id}`} type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submitComment(post.id)} placeholder="Write a comment..." className="w-full bg-zinc-100 dark:bg-zinc-900 rounded-full px-4 py-2.5 text-sm outline-none border border-transparent focus:border-[#9cf822] text-black dark:text-white" />
                            </div>
                            <button onClick={() => submitComment(post.id)} disabled={!commentText.trim()} className="text-[#9cf822] p-2 mb-1"><Send size={18} /></button>
                          </div>
                        </div>
                      )}
                    </div>
        ...
        <div className="lg:col-span-4 space-y-8 order-1 lg:order-2 hidden lg:block">
          <div className="sticky top-24 space-y-8 max-h-[calc(100vh-6rem)] overflow-y-auto no-scrollbar pb-8">
            <div className="w-full bg-gradient-to-br from-[#1a2e05] to-[#0a1401] rounded-[2.5rem] p-8 border border-white/5 shadow-2xl relative overflow-hidden text-left">
              <Sparkles size={18} className="text-[#9cf822] mb-2" /><h3 className="font-bold text-white text-lg tracking-tight uppercase tracking-widest">Community Hub</h3><p className="text-zinc-400 text-sm mb-8 font-medium italic">Join the conversation</p>
              <div className="space-y-6">
                ...
                {trendingTags.map((tag, idx) => (<div key={idx} className="flex flex-col gap-0.5 group cursor-pointer"><span className="text-sm font-black text-white group-hover:text-[#9cf822] transition-colors">{tag.tag}</span><span className="text-[10px] font-bold text-zinc-500 uppercase">{tag.count} Posts</span></div>))}
              </div>
            </div>
               ...
               <div className="space-y-6">
                  {recentActivity.slice(0, 4).map((activity, i) => (
                    <div key={i} className="flex items-start gap-4 group cursor-pointer" onClick={() => router.push(`/profile/${activity.user_id}`)}>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1"><p className="text-sm font-black text-black dark:text-white truncate group-hover:text-[#9cf822] transition-colors">{activity.profiles?.full_name}</p>{activity.profiles?.is_verified && <BadgeCheck size={12} fill="#9cf822" className="text-white shrink-0" />}</div>
