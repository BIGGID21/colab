...
function InboxContent() {
  ...
  const fetchInboxData = async () => {
    ...
    // Process DMs into a unique list of contacts with their latest message
    const contactMap = new Map();
    const uniqueSentReceivers = new Set(); // Track how many unique people I've messaged
    ...
    (dms || []).forEach(dm => {
      ...
      if (!contactMap.has(otherUser.id)) {
        contactMap.set(otherUser.id, {
          user: otherUser,
          lastMessage: dm,
          unread: !isMeSender && !dm.is_read // Basic unread logic
        });
      }
    });
    ...
    // If there's a compose ID, load that chat immediately
    if (composeId) {
      // Check if we already have them in contacts
      const existingContact = sortedContacts.find(c => c.user.id === composeId);
      if (existingContact) {
        handleSelectContact(existingContact.user);
      } else {
        // Fetch their profile and mock a new contact
      ...
      // Auto-select first contact if desktop (we'll keep it simple for now and not auto-select on mobile)
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`dms_${user.id}`)
      .on('postgres_changes', { 
        ...
        // Refresh inbox to get the new message and update the contact list
        fetchInboxData();
        // If we are actively chatting with the sender, add it to the view
        if (activeChatUser && payload.new.sender_id === activeChatUser.id) {
          fetchMessagesForActiveChat(activeChatUser.id);
        }
      })
      .subscribe();
  ...
  const handleSelectContact = (contactUser: any) => {
    ...
    // Clear compose query param without refreshing
    if (composeId) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // --- PAYWALL LOGIC ---
    // Has the user already messaged this specific person before?
    const hasMessagedBefore = contacts.some(c => c.user.id === activeChatUser.id && c.lastMessage.sender_id === user.id);
    
    // If they are free, haven't messaged this person, AND have hit their 3 person limit
    ...
    if (error) {
      ...
      setMessages(prev => prev.filter(m => m.id !== tempMessage.id)); // Revert
    } else {
      triggerHaptic(10);
      fetchInboxData(); // Refresh the contact list to show the new last message
    }
  ...
  const handleUpgradeToPro = async () => {
    // In a real app, this redirects to Stripe/Paystack checkout
    // For now, we mock the upgrade to test it
